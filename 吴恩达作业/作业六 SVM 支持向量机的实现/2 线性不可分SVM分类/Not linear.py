...
def plot_scatter(x1, x2, y):
    ...
    plt.xlabel("x1")
    plt.ylabel("X2")
data2 = loadmat("C:\\Users\\Administrator\\Desktop\\吴恩达作业\\作业六 SVM 支持向量机的实现\\题目\\ex6\\ex6data2.mat")
X = data2["X"]
y = data2["y"]
...
model = svm.SVC(C=1, kernel='rbf', gamma=gamma)#sklean中没有高斯核函数，需要rbf配合gamma实现
model.fit(X, y.ravel())
plot_boundary(model, X, "SVM (Gaussian Kernel) Decision Boundary")
plt.show()